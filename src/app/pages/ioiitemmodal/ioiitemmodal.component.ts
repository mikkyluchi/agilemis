...
@Component({
  selector: 'ioiitemmodal',
  templateUrl: './ioiitemmodal.component.html',
  styleUrls: ['./ioiitemmodal.component.scss']
})
export class IoiitemmodalComponent implements OnInit {

  @Input() src;
  @Input() intermediate_outcome_indicator_component_group_id;
  form: FormGroup;
  states: any;
  lgas: any;
  users: any;
  summaries: any = [];
  contractModalTitle: string = '  Intermediate Outcome Indicator';
  ...
  ngOnInit(): void {
    ...
    if (this.src) {
      ...
      this.contractModalTitle = 'Edit  Intermediate Outcome Indicator';
  ...
  initForm(){
    ...
    this.form = this.fb.group({
      ...
      intermediate_outcome_indicator_component_group_id: [this.intermediate_outcome_indicator_component_group_id || '', Validators.required],
      description:  [this.src?.description || '', Validators.required],
      baseline:  [this.src?.baseline || '', Validators.required],
      target:  [this.src?.target || '', Validators.required],
      calculation: [this.src?.calculation || '', Validators.required]
    })
  }
  ...
  submit(){
    ...
    // }
    if (this.src) {
      this.crud.putCCAuthData('intermediateoutcomeindicator', this.form.value, this.src.id).subscribe(
        (res: any) => {
      ...
      this.crud.postCCCAuthData('intermediateoutcomeindicator', this.form.value).subscribe(
        (res: any) => {
          this.toastr.success(res.payload)
          this.activeModal.close();
        },
