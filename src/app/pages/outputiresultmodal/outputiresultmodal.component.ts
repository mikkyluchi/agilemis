...
@Component({
  selector: 'outputiresultmodal',
  templateUrl: './outputiresultmodal.component.html',
  styleUrls: ['./outputiresultmodal.component.scss']
})
export class OutputiresultmodalComponent implements OnInit {
  ...
  initForm(){
    ...
    this.form = this.fb.group({
      ...
      outcome_indicator_id: [this.src?.outcome_indicator_id || '', Validators.required],
      actual_result: [this.src?.actual_result || '', Validators.required],
      sub_component_id: [this.src?.actual_result || '', Validators.required]
    })
  }
  ...
  submit(){
    ...
    // }
    if (this.src) {
      this.crud.putCCAuthData('outcomes/outcomesedit', this.form.value, this.src.id).subscribe(
        (res: any) => {
      ...
      this.crud.postCCCCCAuthData('outcomes', this.form.value, sessionStorage.getItem('user'),this.currentUserObj.roles[0].name).subscribe(
        (res: any) => {
          this.toastr.success(res.payload)
          this.activeModal.close();
        },
