...
@Component({
  selector: 'intermediateoutcome',
  templateUrl: './intermediateoutcome.component.html',
  styleUrls: ['./intermediateoutcome.component.scss']
})
export class IntermediateoutcomeComponent implements OnInit {
  ...
  getIOIGroups(){
    this.spinner.show();
    this.crud.getCCAuthData('intermediateoutcomeindicatorgroup').subscribe((res: any) => {
  ...
  deleteProjectById() {
    ...
    //     //this.getProjects();
    //   });
    this.crud.deleteCCAuthData('intermediateoutputdelete', this.projectId).subscribe(
      (res: any) => {
            this.toastr.success(res.message);
        document.getElementById('closeDelProjectModal').click();
      },
  ...
  openModal(item: any) {
    const modalRef = this.modalService.open(IoimodalComponent);
