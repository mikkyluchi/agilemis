...
@Component({
  selector: 'crosscuttingissues',
  templateUrl: './crosscuttingissues.component.html',
  styleUrls: ['./crosscuttingissues.component.scss']
})
export class CrosscuttingissuesComponent implements OnInit {
  ...
  submit(){
    ...
    //  return
    // }
    this.crud.postCCCCCAuthData('cissueslist', this.form.value, sessionStorage.getItem('user'), this.user.roles[0].name).subscribe(
      (res: any) => {
  ...
  submitt(){
    ...
    //  return
    // }
    this.crud.postCCCCCAuthData('cissueslistt', this.form.value, sessionStorage.getItem('user'), this.user.roles[0].name).subscribe(
      (res: any) => {
        this.toastr.success(res.payload)
        this.crosscuttingissues = res.data;
      },
  ...
  deleteProjectById() {
    // this.crud.ccdeleteAuthData(`projects/${this.projectId}`)
    //   .subscribe((res: any) => {
    //     this.toastr.success(res.message);
    //     document.getElementById('closeDelProjectModal').click();
    //     //this.getProjects();
    //   });
    this.crud.deleteCCAuthData('cissuesdelete', this.projectId).subscribe(
      (res: any) => {
            this.toastr.success(res.message);
        document.getElementById('closeDelProjectModal').click();
      },
  ...
  openModal(item: any) {
    const modalRef = this.modalService.open(CrosscuttingissuemodalComponent);
